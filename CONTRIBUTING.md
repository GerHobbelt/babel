...
```sh
# Run tests for the @gerhobbelt/babel-plugin-transform-classes package.
...
#### `@gerhobbelt/babel-plugin-x`

All the Babel plugins (and other packages) that have a `/test/fixtures` are written in a similar way.

For example, in [`@gerhobbelt/babel-plugin-transform-exponentiation-operator/test`](https://github.com/babel/babel/tree/master/packages/babel-plugin-transform-exponentiation-operator/test):
...
- There can be multiple folders under [`/fixtures`](https://github.com/babel/babel/tree/master/packages/babel-plugin-transform-exponentiation-operator/test/fixtures)
   ...
   - For this test, we only need the relevant plugin, so it's just `{ "plugins": ["@gerhobbelt/babel-plugin-transform-exponentiation-operator"] }`.
...
{
  "plugins": [["@gerhobbelt/babel-plugin-proposal-object-rest-spread", { "useBuiltIns": "invalidOption" }]],
  "throws": "@gerhobbelt/babel-plugin-proposal-object-rest-spread currently only accepts a boolean option for useBuiltIns (defaults to false)"
}
```
...
#### `@gerhobbelt/babel-parser` (babylon)
...
After writing tests for @gerhobbelt/babel-parser, just build it by running:
...
For both `@gerhobbelt/babel-plugin-x` and `@gerhobbelt/babel-parser`, you can easily generate an `output.js`/`output.json` automatically by just providing `input.js` and running the tests as you usually would.
...
Next, we need to execute `Generator.generate()`, which can be achieved by running a test case in the `@gerhobbelt/babel-generator` package.
