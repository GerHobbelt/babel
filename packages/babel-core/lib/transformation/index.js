  ...
  for (const pluginPairs of pluginPasses) {
    ...
    for (const [plugin, pass] of passPairs) {
      ...
      if (fn) {
        ...
        if (isThenable(result)) {
          throw new Error(`You appear to be using an plugin with an async .pre, ` + `which your current version of Babel does not support.` + `If you're using a published plugin, you may need to upgrade ` + `your @gerhobbelt/babel-core version.`);
        }
      }
    }

    const visitor = _babelTraverse().default.visitors.merge(visitors, passes, file.opts.wrapPluginVisitorMethod);
    ...
    for (const [plugin, pass] of passPairs) {
      ...
      if (fn) {
        ...
        if (isThenable(result)) {
          throw new Error(`You appear to be using an plugin with an async .post, ` + `which your current version of Babel does not support.` + `If you're using a published plugin, you may need to upgrade ` + `your @gerhobbelt/babel-core version.`);
        }
      }
    }
  }
