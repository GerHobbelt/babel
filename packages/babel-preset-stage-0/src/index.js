...
import { proposals } from "@gerhobbelt/babel-plugin-proposal-pipeline-operator";
...
export default declare((api, opts = {}) => {
  ...
  if (typeof loose !== "boolean") {
    throw new Error(
      "@gerhobbelt/babel-preset-stage-0 'loose' option must be a boolean.",
    );
  }
  ...
  if (typeof pipelineProposal !== "string") {
    throw new Error(
      "The pipeline operator requires a proposal set." +
        ...
        " @gerhobbelt/babel-preset-stage-0 whose value must be one of: " +
        proposals.join(", "),
    );
  }
