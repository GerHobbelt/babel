...
function _babelPluginSyntaxJsx() {
  const data = _interopRequireDefault(require("@gerhobbelt/babel-plugin-syntax-jsx"));

  _babelPluginSyntaxJsx = function () {
    return data;
  };
...
var _default = (0, _babelHelperPluginUtils().declare)((api, options) => {
  ...
  const createIdentifierParser = id => () => {
    return id.split(".").map(name => _babelCore().types.identifier(name)).reduce((object, property) => _babelCore().types.memberExpression(object, property));
  };
  ...
  visitor.JSXAttribute = function (path) {
    if (_babelCore().types.isJSXElement(path.node.value)) {
      path.node.value = _babelCore().types.jsxExpressionContainer(path.node.value);
    }
  };
