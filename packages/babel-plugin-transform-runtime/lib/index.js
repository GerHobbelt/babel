    ...
    visitor: {
      ...
      MemberExpression: {
        ...
        exit(path) {
          ...
          if (path.scope.getBindingIdentifier(obj.name)) return;
          path.replaceWith(_babelCore().types.memberExpression(this.addDefaultImport(`${modulePath}/core-js/${definitions.builtins[obj.name]}`, obj.name), node.property, node.computed));
        }
