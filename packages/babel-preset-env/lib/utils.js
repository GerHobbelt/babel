...
const semverify = version => {
  ...
  (0, _assert().default)(typeof version === "number" || typeof version === "string" && versionRegExp.test(version), `'${version}' is not a valid version`);
...
const isPolyfillSource = source => source === "@gerhobbelt/babel-polyfill" || source === "core-js";
...
const createImport = (path, mod) => (0, _babelHelperModuleImports().addSideEffect)(path, getModulePath(mod));
