...
class Tokenizer extends LocationParser {
  ...
  readToken(code) {
    if (this.options.jisonVariables && !this.state.isIterator && !this.state.inType && (code === 35 || code === 64 || code === 36)) {
      ...
      if (this.state.pos === identifierStart && code !== 36) {
        this.raise(chunkStart, `Invalid JISON identifier ${this.input.slice(chunkStart, this.state.pos)}`);
      }
    ...
    while (!this.eat(close)) {
      ...
      if (allowEmpty && this.match(types.comma)) {
        ...
        if (this.state.inFunction && this.state.inParameters && this.match(types.comma)) {
          ...
          const errorMessage = nextTokenType === types.parenR ? "A trailing comma is not permitted after the rest element" : "Rest parameter must be last formal parameter";
...
class ExpressionParser extends LValParser {
  ...
  parseSubscript(base, startPos, startLoc, noCalls, state) {
    if (!noCalls && this.eat(types.doubleColon)) {
      ...
      node.arguments = this.parseCallExpressionArguments(types.parenR, possibleAsync, refTrailingCommaPos);
